...
EOF;
			...
			if ( $reqtype == "passwordreset" && (!($stmt = $_db->prepare("INSERT INTO ResetRequest (Account, EMail, RequestedBy) VALUES(?, ?, ?)")) || !$stmt->bind_param("sss", $account, $email, $ip) || !$stmt->execute())) {
	echo <<<EOF
	<div>
				<h2>Failure</h2>
				<ul>
...
EOF;
			} elseif ( $reqtype == "accountnames" && (!($stmt = $_db->prepare("INSERT INTO ForgotAccount (EMail, RequestedBy) VALUES (?, ?)")) || !$stmt->bind_param("ss", $email, $ip) || !$stmt->execute())) {
	echo <<<EOF
	<div>
	<h2>Failure</h2>
	<ul>
